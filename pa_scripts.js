...
//Quote
function isInsideVeContentColor(e){return null!==e.closest(".ve-content.color")}function expandQuotes(e){if(!isInsideVeContentColor(e)&&!e.querySelector(".quotebtn")){if(e.scrollHeight>170){e.style.height="170px",e.style.maxHeight="none",e.style.overflow="hidden";const t=document.createElement("div");t.className="quotebtn";const o=document.createElement("button");o.innerHTML="Show More...",o.type="button",t.appendChild(o),e.appendChild(t)}else if(e.querySelector(".spoiler")){e.style.maxHeight="none"}}}function modifyQuoteTop(e){if(isInsideVeContentColor(e))return;const t=e.textContent,o=e.querySelector("a");if(t.includes("@")){const n=t.replace(/(.*)\s*\(([^@]+)@[^)]+\)\s*/,"$2 said:");e.innerHTML=n,e.style.color="var(--mdcol)",o&&(e.appendChild(o),o.style.color="var(--mdcol)")}else{const t=e.querySelector(".quote_top b");t&&(t.style.opacity=1)}}function animateHeight(e,t,o){const n=e.clientHeight,c=performance.now();requestAnimationFrame((function animate(i){const r=i-c,u=Math.min(r/o,1);e.style.height=n+(t-n)*u+"px",u<1?requestAnimationFrame(animate):(e.style.height="auto",e.style.overflow="visible")}))}document.body.addEventListener("click",(function(e){if(e.target.matches(".quotebtn button")){const t=e.target.closest(".quote");animateHeight(t,t.scrollHeight,382),setTimeout((function(){e.target.parentElement.remove()}),0)}})),document.querySelectorAll(".color .quote, .st-emoji-post-content .color .quote").forEach(expandQuotes),document.querySelectorAll(".color .quote_top, .st-emoji-post-content .color .quote_top").forEach(modifyQuoteTop);const quot=new MutationObserver((function(e){e.forEach((function(e){e.addedNodes.forEach((function(e){e.nodeType===Node.ELEMENT_NODE&&(e.matches(".quote")&&expandQuotes(e),e.querySelectorAll(".quote").forEach((function(e){e.querySelector(".quotebtn")||expandQuotes(e)})),e.matches(".quote_top")&&modifyQuoteTop(e),e.querySelectorAll(".quote_top").forEach(modifyQuoteTop))}));for(const t of e.target.querySelectorAll?.(".quote")||[])t.querySelector(".quotebtn")||expandQuotes(t);for(const t of e.target.querySelectorAll?.(".quote_top")||[])modifyQuoteTop(t)}))}));quot.observe(document.body,{childList:!0,subtree:!0,attributes:!0}),new MutationObserver((function(e){e.forEach((function(e){for(const t of e.target.querySelectorAll?.(".quote")||[])t.querySelector(".quotebtn")||expandQuotes(t)}))})).observe(document.body,{attributes:!0,subtree:!0,attributeFilter:["src"]});
//Skin
...
function initTimestampScript(){
    ...
    console.log("User timezone:", t);
    console.log("Is guest:", e);
    
    function i(t){
        var i=String(t).replace(/:(\d{2})$/,"").trim();
        if(e){
            var r=["D/M/YYYY, HH:mm","D MMM YYYY, HH:mm","D MMMM YYYY, HH:mm","YYYY/MM/DD HH:mm"];
        ...
        var m=/AM|PM/i.test(i);
        var l=m?["M/D/YYYY, h:mm A","M/D/YYYY","MMM D, YYYY, h:mm A","MMMM D, YYYY, h:mm A"]:["D/M/YYYY, HH:mm","D/M/YYYY","D MMM YYYY, HH:mm","D MMMM YYYY, HH:mm"];
        for(var c=0;c<l.length;c++){
            var s=moment(i,l[c],true);
            if(s.isValid())return s
        }
        if(moment(i,moment.ISO_8601,true).isValid()){
            return moment(i,moment.ISO_8601,true)
        }
        var u=["YYYY-MM-DD HH:mm","YYYY/MM/DD HH:mm","MMMM D, YYYY","MMM D, YYYY","YYYY MMMM D","YYYY MMM D"];
        for(var d=0;d<u.length;d++){
            var f=moment(i,u[d],true);
    ...
    function r(t){
        ...
        var e=moment();
        var i=e.diff(t,"hours");
        
        // DEBUG: Log the relative time calculation
        console.log("=== RELATIVE TIME DEBUG ===");
        console.log("Input date:", t.format());
        console.log("Input date local:", t.local().format());
        console.log("Current time:", e.format());
        console.log("Hour difference:", i);
        console.log("Day names - input:", t.format("dddd"), "current:", e.format("dddd"));
        
        if(i<24){
            var result = t.fromNow();
            console.log("Result: '"+result+"' (less than 24h)");
            return result;
        }else if(i<48){
            var result = "Yesterday at "+t.format("h:mm A");
            console.log("Result: '"+result+"' (yesterday)");
            return result;
        }else if(i<168){
            var result = t.format("dddd [at] h:mm A");
            console.log("Result: '"+result+"' (this week)");
            return result;
        }else{
            var result = t.format("MMM D, YYYY");
            console.log("Result: '"+result+"' (absolute date)");
            return result;
        }
    }
    ...
    function a(e,i){
        try{
            ...
            console.log("Input format:", i);
            console.log("Source timezone: Europe/Rome");
            console.log("Target timezone:", t);
            
            var r=moment.tz(e,i,"Europe/Rome");
            console.log("After Rome parsing:", r.format());
            
            if(!r.isValid()){
                console.log("❌ Rome parsing failed");
                return null
            }
            
            var converted = r.clone().tz(t);
            console.log("After conversion to user tz:", converted.format());
            console.log("Conversion successful:", converted.isValid());
            
            return converted;
        }catch(t){
            console.log("❌ Timezone conversion error:", t);
            return null
        }
    }
    ...
    function n(t){
        try{
            ...
            const i=t.querySelector(".d_month");
            const r=t.querySelector(".d_year");
    ...
    function d(t){
        try{
            ...
            var c=t.querySelector(".d_month")?t.querySelector(".d_month").textContent.trim():"";
            var s=t.querySelector(".d_year")?t.querySelector(".d_year").textContent.trim():"";
            c=c.replace(/([A-Za-z]+).*/,"$1");
            var u=s+"-"+c+"-"+l;
            var e=moment(u,"YYYY-MMM-DD",true);
            if(!e.isValid()){
                e=i(u)
            }
            if(e.isValid()){
                ...
                a.setAttribute("dir","auto");
                a.setAttribute("datetime",e.format());
                a.setAttribute("title",e.format("MMM D, YYYY [at] h:mm A"));
                a.textContent=r(e);
                a.style.visibility="visible";
                t.replaceWith(a)
            }
    ...
    function y(t){
        try{
            console.log("=== SLIDER TIMESTAMP DEBUG (y function) ===");
            console.log("Slider element:", t);
            console.log("Slider HTML:", t.outerHTML);
            
            if(!document.body.matches("#board"))return;
            var e=t.lastChild;
            if(!e||e.nodeType!==Node.TEXT_NODE)return;
            var i=e.textContent.trim();
            console.log("Raw slider text:", i);
            
            var o=a(i,"D/M/YYYY, HH:mm");
            console.log("After timezone conversion:", o ? o.format() : "null");
            ...
            console.log("=== SLIDER PROCESSING COMPLETE ===");
        }catch(t){
            console.log("❌ Slider processing error:", t);
            return;
        }
    }
    ...
    function b(t){
        try{
            console.log("=== SLIDER TIMESTAMP DEBUG (b function) ===");
            console.log("Lastarticles element:", t);
            console.log("Lastarticles HTML:", t.outerHTML);
            
            var e=t.lastChild;
            if(!e||e.nodeType!==Node.TEXT_NODE)return;
            var i=e.textContent.trim();
            console.log("Raw lastarticles text:", i);
            ...
            console.log("=== LASTARTICLES PROCESSING COMPLETE ===");
        }catch(t){
            console.log("❌ Lastarticles processing error:", t);
            return;
        }
    }
    ...
    function M(t){
        try{
            ...
            if(t.classList.contains("st-emoji-epost-time")){
                var i=moment(e,"YYYY/MM/DD HH:mm",true);
                if(i&&i.isValid()){
                    t.textContent=r(i);
    ...
    function A(t,e){
        try{
            ...
            if(e===".big_list .zz .when"&&!document.body.matches("#board, #forum, #blog, #search")){
                return
            }
            if(e===".post .title2.top .when"&&!document.body.matches("#topic, #search, #blog")){
                return
            }
            if(e===".summary .when"&&!document.body.matches("#send")){
                return
            }
            if(e===".article .title2.top .when"&&!document.body.matches("#blog")){
                return
            }
            var o=t.getAttribute("title")||t.textContent.trim();
            if(t.children.length&&t.children[0].tagName==="SPAN"){
                o=t.childNodes[t.childNodes.length-1].textContent.trim()
            }
            var a=i(o);
            if(a.isValid()){
                if(t.classList.contains("st-emoji-notice-time")||t.classList.contains("st-emoji-epost-time")){
                    M(t)
                }else{
                    var n=document.createElement("time");
                    n.className="u-dt";
                    if(t.classList.contains("when")){
                        n.classList.add("when")
                    }
    ...
    function p(){
        try{
            ...
            var t=[".big_list .zz .when",".post-date",".time",".date",".post .title2.top .when",".summary .when"];
        ...
        try{
            t.forEach((function(t){
                t.addedNodes.forEach((function(t){
                    if(t.nodeType===1){
                        if(t.matches("dl.profile-joined, dl.profile-lastaction")){
                            s(t)
                        }
                        t.querySelectorAll("dl.profile-joined, dl.profile-lastaction").forEach(s);
                        if(t.matches(".timeago")){
                            l(t)
                        }
                        t.querySelectorAll(".timeago").forEach(l);
                        if(t.matches(".post .edit")){
                            c(t)
                        }
                        t.querySelectorAll(".post .edit").forEach(c);
                        if(t.matches(".st-emoji-post-time")){
                            v(t)
                        }
                        t.querySelectorAll(".st-emoji-post-time").forEach(v);
                        if(document.body.matches("#online")&&t.matches(".online .yy .when")){
                            u(t)
                        }
                        if(document.body.matches("#online")){
                            t.querySelectorAll(".online .yy .when").forEach(u)
                        }
                        if(document.body.matches("#blog")&&t.matches(".article .title2.top .when")){
                            d(t)
                        }
                        if(document.body.matches("#blog")&&t.matches(".bt_mini .when")){
                            f(t)
                        }
                        if(document.body.matches("#blog")&&t.matches(".mini_buttons .when")){
                            h(t)
                        }
                        if(document.body.matches("#blog")){
                            t.querySelectorAll(".article .title2.top .when").forEach(d);
                            t.querySelectorAll(".bt_mini .when").forEach(f);
                            t.querySelectorAll(".mini_buttons .when").forEach(h)
                        }
                        if(document.body.matches("#group, #members")&&t.matches(".big_list .cc")){
                            Y(t)
                        }
                        if(document.body.matches("#group, #members")){
                            t.querySelectorAll(".big_list .cc").forEach(Y)
                        }
                        if(document.body.matches("#board")&&t.matches(".side_topics .when")){
                            y(t)
                        }
                        if(document.body.matches("#board")){
                            t.querySelectorAll(".side_topics .when").forEach(y)
                        }
                        if(t.matches(".lastarticles .topic .when")){
                            b(t)
                        }
                        t.querySelectorAll(".lastarticles .topic .when").forEach(b);
                        if(t.matches(".st-emoji-epost-time, .st-emoji-notice-time")){
                            M(t)
                        }
                        t.querySelectorAll(".st-emoji-epost-time, .st-emoji-notice-time").forEach(M);
                        var e=[".big_list .zz .when",".post-date",".time",".date",".post .title2.top .when",".summary .when"];
