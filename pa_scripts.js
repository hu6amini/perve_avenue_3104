...
//Quote
function isInsideVeContentColor(e){return null!==e.closest(".ve-content.color")}function expandQuotes(e){if(isInsideVeContentColor(e))return;const updateHeight=()=>{const o=e.querySelector(".quotebtn button");if(!o&&e.scrollHeight>170){e.style.maxHeight="170px";const o=document.createElement("div");o.className="quotebtn";const t=document.createElement("button");t.innerHTML="Show More...",t.type="button",o.appendChild(t),e.appendChild(o)}else o&&e.scrollHeight<=170&&o.parentNode.remove()};updateHeight();const o=new ResizeObserver(updateHeight);o.observe(e);const t=e.querySelector(".spoiler .code_top a");t&&t.addEventListener("click",()=>{e.style.maxHeight="none",o.disconnect()})}document.querySelectorAll(".quote").forEach(expandQuotes),document.body.addEventListener("click",function(e){if(e.target&&e.target.matches(".quotebtn button")){const t=e.target.closest(".quote");t&&(t.style.transition="max-height 0.382s ease-in-out",t.style.maxHeight=t.scrollHeight+"px",e.target.parentNode.style.display="none",setTimeout(()=>{t.style.maxHeight="none"},382))}},!1);const quoteObserver=new MutationObserver(e=>{e.forEach(e=>{e.addedNodes.forEach(e=>{if(e.nodeType===Node.ELEMENT_NODE)if(e.matches(".quote"))expandQuotes(e);else e.querySelectorAll(".quote").forEach(expandQuotes)})})});quoteObserver.observe(document.body,{childList:!0,subtree:!0}),document.querySelectorAll(".quote_top").forEach(modifyQuoteTop);const quoteTopObserver=new MutationObserver(e=>{e.forEach(e=>{e.addedNodes.forEach(e=>{if(e.nodeType===Node.ELEMENT_NODE)e.querySelectorAll(".quote_top").forEach(modifyQuoteTop)})})});function modifyQuoteTop(e){if(isInsideVeContentColor(e))return;const o=e.textContent,t=e.querySelector("a");if(o.includes("@")){const n=o.replace(/(.*)\s*\(([^@]+)@[^)]+\)\s*/,"$2 said:");e.innerHTML=n,e.style.color="var(--mdcol)",t&&(e.appendChild(t),t.style.color="var(--mdcol)")}else{const o=e.querySelector(".quote_top b");o&&(o.style.opacity=1)}}quoteTopObserver.observe(document.body,{childList:!0,subtree:!0});
//Youtube lite
