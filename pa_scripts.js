...
//Image generator
function createCustomDialog(){const e=document.querySelector(".imgen-backdrop"),t=document.querySelector(".imgen"),n=document.querySelector(".imgen-alert");if(t){t.style.display="block",e.style.display="block",n.style.display="none";const a=t.querySelector("input");a&&a.focus()}else{const e=document.createElement("div");e.className="imgen",e.style.zIndex=9999;const t=document.createElement("div");t.className="imgen-backdrop",t.addEventListener("click",function(){e.style.display="none",t.style.display="none",document.querySelector(".imgen-alert").style.display="none",document.querySelector(".imgen input").value=""});const n=document.createElement("div");n.className="header",n.innerHTML='<i class="fa-regular fa-image"></i> Generate Image Thumbnail',e.appendChild(n);const a=document.createElement("div");a.className="imgen-alert",a.innerHTML='<i class="fa-regular fa-circle-exclamation"></i> Please enter a valid URL',e.appendChild(a),a.style.display="none";const l=document.createElement("input");l.type="text",l.placeholder="Insert image link...",l.setAttribute("aria-label","Image URL"),e.appendChild(l);const c=document.createElement("button");c.className="codebuttons",c.textContent="INSERT",c.addEventListener("click",function(){const n=l.value;validateImageURL(n,function(valid){if(valid){const img=document.createElement("img");img.src=n,img.alt="Generated Image",document.querySelector('textarea#Post').value+="<img src=\""+n+"\">",triggerInputEvent(),e.style.display="none",t.style.display="none",l.value=""}else a.style.display="block"})});e.appendChild(c),c.setAttribute("aria-label","Insert"),l.addEventListener("click",function(){l.select()}),document.body.appendChild(t),document.body.appendChild(e),e.style.display="block",l.focus()}}function triggerInputEvent(){const e=document.querySelector("textarea#Post");e&&e.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0}))}function validateImageURL(url,callback){const img=new Image;img.src=url,img.onload=function(){callback(!0)},img.onerror=function(){callback(!1)}}function tag_image(){createCustomDialog()}document.URL.includes("?act=Post")||document.URL.includes("?t=")||document.querySelector('.codebuttons[value*="IMG"]')&&document.querySelector('.codebuttons[value*="IMG"]').addEventListener("click",tag_image);
//Link generator
...
//Textarea image santizer
document.addEventListener("click",e=>{e.target.closest('a[href^="/?act=Post&CODE=08"], a[href^="/?act=Post&CODE=02"]')&&localStorage.setItem("stripLZ","true")});function wrapImagesWithLZ(){const e=document.querySelector("#Post");if(!e)return;let t=document.createElement("div");t.innerHTML=e.value,t.querySelectorAll("img").forEach(function(e){if(!e.closest(".lz-container")){let t=e.getAttribute("src"),a=e.naturalWidth||0,r=e.naturalHeight||0;if(t&&a&&r){let n=document.createElement("div");n.className="lz-container",n.setAttribute("width",a),n.setAttribute("height",r);let i=document.createElement("a");i.href=t,i.target="_blank";let l=document.createElement("img");l.className="color_img lazyload",l.setAttribute("data-src",t),l.setAttribute("decoding","async"),l.setAttribute("width",a),l.setAttribute("height",r),i.appendChild(l),n.appendChild(i),e.replaceWith(n)}}}),e.value=t.innerHTML}document.querySelectorAll('.send .Item[style="padding: 6px 2%"] .Justify .right.Sub button[type="submit"],.send .Item[style="padding: 6px 2%"] .Justify .right.Sub input[type="submit"],.send .darkbar.Item button[type="submit"],.send .darkbar.Item input[type="submit"]').forEach(e=>{e.addEventListener("click",()=>{wrapImagesWithLZ()})});function stripLZContainers(){var e=document.querySelector("textarea#Post");if(e){var t=document.createElement("div");t.innerHTML=e.value,t.querySelectorAll(".lz-container").forEach(function(e){var t=e.querySelector("img.color_img");if(t){var a=document.createElement("img");a.src=t.getAttribute("data-src")||t.src,e.replaceWith(a)}}),e.value=t.innerHTML,e.dispatchEvent(new Event("input",{bubbles:!0,cancelable:!0}))}}"send"===document.body.id?(localStorage.getItem("stripLZ")&&localStorage.removeItem("stripLZ"),stripLZContainers()):document.addEventListener("DOMContentLoaded",()=>{"send"===document.body.id&&stripLZContainers()});
//Loading icon
