...
//Quote
function isInsideVeContentColor(e){return null!==e.closest(".ve-content.color")}function expandQuotes(e){if(isInsideVeContentColor(e))return;if(e.querySelector(".quotebtn"))return;if(e.scrollHeight>170){e.style.maxHeight="170px",e.style.overflow="hidden",e.style.transition="max-height 0.382s ease-in-out";const t=document.createElement("div");t.className="quotebtn";const n=document.createElement("button");n.innerHTML="Show More...",n.type="button",n.setAttribute("onclick",`const e=this.closest('.quote');e.style.maxHeight=e.scrollHeight+'px';setTimeout(()=>{e.style.maxHeight='none',e.style.overflow='visible',this.parentElement.remove()},382);`),t.appendChild(n),e.appendChild(t)}}document.querySelectorAll(".color .quote").forEach(expandQuotes);const quoteObserver=new MutationObserver(e=>{e.forEach(e=>{e.addedNodes.forEach(e=>{if(e.nodeType===Node.ELEMENT_NODE){if(e.matches(".quote"))expandQuotes(e);e.querySelectorAll(".quote").forEach(t=>{t.querySelector(".quotebtn")||expandQuotes(t)})}}})});quoteObserver.observe(document.body,{childList:!0,subtree:!0});document.querySelectorAll(".color .quote_top").forEach(e=>{if(isInsideVeContentColor(e))return;const t=e.textContent,n=e.querySelector("a");if(t.includes("@")){const o=t.replace(/(.*)\s*\(([^@]+)@[^)]+\)\s*/,"$2 said:");e.innerHTML=o,e.style.color="var(--mdcol)",n&&(e.appendChild(n),n.style.color="var(--mdcol)"))}else{const t=e.querySelector(".quote_top b");t&&(t.style.opacity=1)}});const quoteTopObserver=new MutationObserver(e=>{e.forEach(e=>{e.addedNodes.forEach(e=>{e.nodeType===Node.ELEMENT_NODE&&e.querySelectorAll(".quote_top").forEach(t=>{if(isInsideVeContentColor(t))return;const e=t.textContent,n=t.querySelector("a");if(e.includes("@")){const o=e.replace(/(.*)\s*\(([^@]+)@[^)]+\)\s*/,"$2 said:");t.innerHTML=o,t.style.color="var(--mdcol)",n&&(t.appendChild(n),n.style.color="var(--mdcol)"))}else{const e=t.querySelector(".quote_top b");e&&(e.style.opacity=1)}})})})});quoteTopObserver.observe(document.body,{childList:!0,subtree:!0});
//Youtube lite
