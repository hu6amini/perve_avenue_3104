...
document.addEventListener("DOMContentLoaded", () => {
    function isInsideVeContentColor(element) {
        return element.closest('.ve-content.color') !== null;
    }

    function expandQuotes(e) {
        if (isInsideVeContentColor(e)) return;

        const updateHeight = () => {
            const t = e.querySelector(".quotebtn button");
            if (!t && e.scrollHeight > 170) {
                e.style.maxHeight = "170px";
                const t = document.createElement("div");
                t.className = "quotebtn";
                const o = document.createElement("button");
                o.innerHTML = "Show More...";
                t.appendChild(o);
                e.appendChild(t);
                o.addEventListener("click", (o) => {
                    o.preventDefault();
                    o.stopPropagation();
                    e.style.transition = "max-height 0.382s ease-in-out";
            ...
            } else if (t && e.scrollHeight <= 170) {
                t.parentNode.remove();
            }
        };

        updateHeight();
        const t = new ResizeObserver(updateHeight);
        t.observe(e);
        const o = e.querySelector(".spoiler .code_top a");
    ...
    function modifyQuoteTop(e) {
        ...
        if (t.includes("@")) {
            const n = t.replace(/QUOTE\s*\(([^@]+)@[^)]+\)\s*/, "$1 said:");
            e.innerHTML = n;
            e.style.color = "var(--mdcol)";
            o && (e.appendChild(o), o.style.color = "var(--mdcol)");
        } else {
            const t = e.querySelector(".quote_top b");
            t && (t.style.opacity = 1);
        }
    }

    (function initializeExpandQuotes() {
        document.querySelectorAll(".quote").forEach(expandQuotes);
        new MutationObserver((e) => {
            for (const t of e) {
                if (t.addedNodes.length > 0) {
                    t.addedNodes.forEach((e) => {
                        if (e.nodeType === Node.ELEMENT_NODE && !isInsideVeContentColor(e)) {
                            e.classList.contains("quote") ? expandQuotes(e) : e.querySelectorAll(".quote").forEach(expandQuotes);
                        }
                    });
                }
            }
        ...
        new MutationObserver((e) => {
            for (const t of e) {
                if (t.type === "childList") {
                    t.addedNodes.forEach((e) => {
                        if (e.nodeType === Node.ELEMENT_NODE && !isInsideVeContentColor(e)) {
                            e.querySelectorAll(".quote_top").forEach(modifyQuoteTop);
                        }
                    });
                }
            }
