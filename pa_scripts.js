...
$('.slick_carousel').on('beforeChange', function(event, slick, currentSlide, nextSlide) {
    let $nextSlide = $(slick.$slides[nextSlide]); // Get the next slide

    // Find images & sources inside the next slide and trigger LazySizes
    $nextSlide.find('source, img').each(function() {
        if ($(this).attr('data-srcset') || $(this).attr('data-src')) {
            $(this).addClass('lazyload'); // Forces LazySizes to load
        }
    });
});
...
$('.slick_carousel').slick({
    ...
    centerMode: true,
    pauseOnFocus: false,
    lazyLoad: 'ondemand' // Let Slick handle lazy loading too
});
//Favicons 
