...
//Profile tooltips
async function fetchUserDetails(e){try{const t=await fetch(e);if(!t.ok)throw new Error('Failed to fetch '+e);const n=await t.arrayBuffer(),o=new TextDecoder('windows-1252').decode(n),a=(new DOMParser).parseFromString(o,'text/html'),r=a.querySelector('.profile span.avatar')?.innerHTML||'',d=a.querySelector('.profile span.nick')?.textContent||'Unknown User',l=a.querySelector('.profile-group dd.Sub.Item')?.textContent.trim()||'',c=a.querySelector('.profile-title dd.Sub.Item')?.textContent.trim()||'',i=a.querySelector('.profile-location dd.Sub.Item')?.textContent.trim()||'',u=e=>e.replace(/^\+/,'').replace(/\s*\(.*?\)/g,'').trim(),m=e=>e.split(',')[0].trim(),p=a.querySelector('.profile-joined')?{label:a.querySelector('.profile-joined dt')?.textContent.trim()||'Joined',value:m(a.querySelector('.profile-joined dd')?.textContent||'')}:null,s=a.querySelector('.profile-posts')?{label:a.querySelector('.profile-posts dt')?.textContent.trim()||'Posts',value:u(a.querySelector('.profile-posts dd')?.textContent||'')}:null,f=a.querySelector('.profile-rep.u_reputation')?{label:a.querySelector('.profile-rep.u_reputation dt')?.textContent.trim()||'Reputation',value:u(a.querySelector('.profile-rep.u_reputation dd')?.textContent||'')}:null,g=a.querySelector('.profile-lastaction')?{label:a.querySelector('.profile-lastaction dt')?.textContent.trim()||'Last Action',value:m(a.querySelector('.profile-lastaction dd')?.textContent||'')}:null;return{avatar:r,nick:d,group:l,title:c,loc:i,joined:p,posts:s,rep:f,lastAction:g}}catch(e){return console.error('Error fetching user details: '+e),null}}function initUserTooltips(){document.querySelectorAll('.post .nick a, .post .avatar, .big_list .who a, .big_list .avatar, .tag .nickname, .side_topics .who a, .lastarticles .topic > span a').forEach(e=>{tippy(e,{allowHTML:!0,placement:'bottom',theme:'dark',animation:'fade',offset:[0,10],arrow:!1,onShow(t){e.dataset.tooltipLoaded||(t.popper.classList.add('hidden'),fetchUserDetails(e.href).then(n=>{if(!n)return t.setContent('<div class="tooltip-error">Error loading user info</div>'),e.dataset.tooltipLoaded='true',t.popper.classList.remove('tippy-profile','hidden'),void t.popper.classList.add('tippy-profile-error');let o='',a='';o=n.group.toLowerCase().includes('global moderator')?'global-moderator':n.group.toLowerCase().includes('administrator')?'administrator':n.group.toLowerCase().includes('moderator')?'moderator':n.group.toLowerCase().includes('game dev')?'game-dev':'utente',a=o;const r='<div class="tooltip-user-details"><div class="tippy-details"><div class="tooltip-avatar">'+n.avatar+'</div><div class="tippy-info"><div class="tippy-nick '+a+'">'+n.nick+'</div>'+(n.group?'<div class="tippy-group '+o+'">'+n.group+'</div>':'')+(n.title?'<div class="tippy-title">'+n.title+'</div>':'')+(n.loc?'<div class="tippy-location">'+n.loc+'</div>':'')+'</div></div><div class="tippy-score">'+(n.posts?'<div class="tippy-posts"><span class="tippy-posts-label">'+n.posts.label+'</span><span class="tippy-posts-value">'+n.posts.value+'</span></div>':'')+(n.rep?'<div class="tippy-rep"><span class="tippy-rep-label">'+n.rep.label+'</span><span class="tippy-rep-value">'+n.rep.value+'</span></div>':'')+(n.joined?'<div class="tippy-joined"><span class="tippy-joined-label">'+n.joined.label+'</span><span class="tippy-joined-value">'+n.joined.value+'</span></div>':'')+(n.lastAction?'<div class="tippy-last-action"><span class="tippy-last-action-label">'+n.lastAction.label+'</span><span class="tippy-last-action-value">'+n.lastAction.value+'</span></div>':'')+'</div></div>';t.setContent(r),e.dataset.tooltipLoaded='true',t.popper.classList.remove('hidden')})}},onCreate(e){e.popper.classList.add('tippy-profile','hidden')}})})}initUserTooltips();
//Emojione 
