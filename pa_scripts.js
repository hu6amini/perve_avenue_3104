...
//Quote
function isInsideVeContentColor(e){return null!==e.closest(".ve-content.color")}function expandQuotes(e){if(!isInsideVeContentColor(e)&&!e.querySelector(".quotebtn")&&e.scrollHeight>170){e.style.maxHeight="170px",e.style.overflow="hidden",e.style.transition="max-height 0.382s ease-in-out";const o=document.createElement("div");o.className="quotebtn";const t=document.createElement("button");t.innerHTML="Show More...",t.type="button",t.addEventListener("click",function(){const quote=this.closest(".quote");quote.style.maxHeight=quote.scrollHeight+"px",setTimeout(()=>{quote.style.maxHeight="none",quote.style.overflow="visible",this.parentElement.remove()},382)}),o.appendChild(t),e.appendChild(o)}}document.querySelectorAll(".color .quote").forEach(expandQuotes);const quoteObserver=new MutationObserver(mutations=>{mutations.forEach(mutation=>{mutation.addedNodes.forEach(node=>{if(node.nodeType===Node.ELEMENT_NODE){node.matches(".quote")&&expandQuotes(node),node.querySelectorAll(".quote").forEach(quote=>{quote.querySelector(".quotebtn")||expandQuotes(quote)})}})})});quoteObserver.observe(document.body,{childList:!0,subtree:!0});document.querySelectorAll(".color .quote_top").forEach(modifyQuoteTop);const quoteTopObserver=new MutationObserver(mutations=>{mutations.forEach(mutation=>{mutation.addedNodes.forEach(node=>{if(node.nodeType===Node.ELEMENT_NODE)node.querySelectorAll(".quote_top").forEach(modifyQuoteTop)})})});quoteTopObserver.observe(document.body,{childList:!0,subtree:!0});function modifyQuoteTop(e){if(isInsideVeContentColor(e))return;const o=e.textContent,t=e.querySelector("a");if(o.includes("@")){const n=o.replace(/(.*)\s*\(([^@]+)@[^)]+\)\s*/,"$2 said:");e.innerHTML=n,e.style.color="var(--mdcol)",t&&(e.appendChild(t),t.style.color="var(--mdcol)")}else{const o=e.querySelector(".quote_top b");o&&(o.style.opacity=1)}}
//Youtube lite
